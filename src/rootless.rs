...
pub fn rootless_chroot<'a>(args: Args<'a>) {
...
fn rootless_chroot_ptrace<'a>(mut args: Args<'a>) {
    ...
    // We must not guess syscall entry/exit by "toggling" on every stop: signals, seccomp traps,
    // and single-stepping can introduce extra stops and desynchronize the toggle. If we ever
    // run the "entry" rewrite on an exit stop, we can corrupt return values (e.g. set x0=-100,
    // which user-space reports as ENETDOWN). Prefer PTRACE_GET_SYSCALL_INFO when available.
    let mut in_syscall_fallback = false;
    let do_rewrite = true;
    // PRoot supports a "fake root" mode (-0) by faking uid/gid syscalls. This is not chroot,
    // but it is required to run many distro tools (e.g. pacman sync/install) under a rootless
    // environment where the real euid != 0.
    let fake_root = std::env::var("PTRACE_PLAYGROUND_FAKE_ROOT")
        .ok()
    ...
    loop {
        match wait::waitpid(pid, None) {
            Ok(wait::WaitStatus::PtraceSyscall(_)) => {
                ...
                let is_entry = ptrace_syscall_is_entry(pid, &mut in_syscall_fallback);
                ...
                if let Ok(regs) = read_regs(pid) {
                    ...
                    if do_rewrite && is_entry && is_guest {
                        ...
                        if let Err(e) = rewrite_syscall_path_with_regs(pid, regs, &mappings, &rootfs_abs_s) {
                            eprintln!("rewrite error: {e}");
                        }
                    }

                    // If enabled, fake "root identity" by overriding uid/gid return values.
                    // We do this on syscall-exit stops only, so x0 is the return value.
                    if fake_root && !is_entry && is_guest {
                        match regs.regs[8] as i64 {
                            ...
                            | nix::libc::SYS_getegid => {
                                let mut regs2 = regs;
                                regs2.regs[0] = 0;
                                if let Err(e) = write_regs(pid, &regs2) {
                                    eprintln!("fake-root write_regs failed: {e}");
                                }
                            }
                            // Pacman and other distro tools may attempt to chown/chmod even when
                            // installing into a user-owned rootfs tree. Under a real PRoot "-0"
                            // mode these are typically ignored. Best-effort: turn EPERM into success.
                            nix::libc::SYS_fchownat
                            | nix::libc::SYS_fchown
                            | nix::libc::SYS_fchmodat
                            | nix::libc::SYS_fchmod => {
                                let rv = regs.regs[0] as i64;
                                if rv == -(nix::errno::Errno::EPERM as i64) {
                                    let mut regs2 = regs;
                                    regs2.regs[0] = 0;
                                    let _ = write_regs(pid, &regs2);
                                }
                            }
                            _ => {}
                        }
                    ...
                    if !is_entry && is_guest && (regs.regs[8] as i64) == nix::libc::SYS_set_robust_list {
            ...
            Ok(wait::WaitStatus::Stopped(_, sig)) => {
                ...
                if sig == nix::sys::signal::Signal::SIGSYS {
                    ...
                    // On Android, SECCOMP_RET_TRAP commonly manifests as SIGSYS. Rather than
                    // depending on guest signal handlers (which may be overwritten by glibc),
                    // emulate a small set of probed syscalls directly in the tracer by:
                    // - setting x0 (return value)
                    // - advancing PC past the `svc` instruction
                    // - resuming without delivering SIGSYS
                    // Only emulate faccessat2 here. We keep set_robust_list/rseq handling in the
                    // loader-shim SIGSYS handler, since glibc's bring-up is sensitive to details.
                    if let Ok(mut regs) = read_regs(pid) {
                        let sysno = regs.regs[8] as i64;
                        if sysno == 439 {
                            regs.regs[0] = 0; // pretend success
                            regs.pc = regs.pc.wrapping_add(4); // skip `svc`
                            let _ = write_regs(pid, &regs);
                            ptrace::syscall(pid, None).unwrap();
                            continue;
                        }
                    }
...
fn rewrite_syscall_path(pid: Pid, mappings: &[(String, String)], rootfs_abs: &str) -> nix::Result<()> {
    let regs = read_regs(pid)?;
    rewrite_syscall_path_with_regs(pid, regs, mappings, rootfs_abs)
}
...
fn rewrite_syscall_path_with_regs(
    ...
    fn log_paths_enabled() -> bool {
        use std::sync::OnceLock;
        static ON: OnceLock<bool> = OnceLock::new();
        *ON.get_or_init(|| std::env::var("PTRACE_PLAYGROUND_LOG_PATHS").is_ok())
    }
    ...
    // Android bionic/libc headers don't always expose every SYS_* constant (notably fstatat),
    // but the kernel ABI is stable for aarch64.
    #[cfg(target_arch = "aarch64")]
    const SYS_FSTATAT: i64 = 79;
    ...
    let path_addr = match syscall as i64 {
        ...
        nix::libc::SYS_linkat => Some(args[1] as usize),   // oldpath
        nix::libc::SYS_renameat => Some(args[1] as usize), // oldpath
        nix::libc::SYS_renameat2 => Some(args[1] as usize), // oldpath
        // `chdir("/")` is a common pattern to "reset" cwd. Without translating it, tracees will
        // escape to the host `/` because this is not a real kernel chroot.
        nix::libc::SYS_chdir => Some(args[0] as usize),
        // glibc's `stat(2)` family typically routes through fstatat/newfstatat (aarch64: 79).
        #[cfg(target_arch = "aarch64")]
        SYS_FSTATAT => Some(args[1] as usize),
        _ => None,
    };
    ...
    // If the tracee is already using a host-absolute path *inside* the rootfs directory
    // (e.g. glibc building paths from getcwd()), don't rewrite it again.
    if path.starts_with(rootfs_abs) {
        return Ok(());
    }
    ...
    if log_paths_enabled() {
        eprintln!("rewrite: nr={} path='{}' -> '{}'", syscall, path, mapped);
    }
    ...
    if made_relative {
        let sc = syscall as i64;
        // For *at syscalls, absolute paths ignore dirfd. If we rewrite an absolute path to a
        // cwd-relative one, we must also force dirfd=AT_FDCWD to preserve semantics.
        let is_at = sc == nix::libc::SYS_openat
            || sc == nix::libc::SYS_openat2
            || sc == nix::libc::SYS_execveat
            || sc == nix::libc::SYS_statx
            || sc == nix::libc::SYS_faccessat
            || sc == nix::libc::SYS_faccessat2
            || sc == nix::libc::SYS_readlinkat
            || sc == nix::libc::SYS_mkdirat
            || sc == nix::libc::SYS_unlinkat
            || sc == nix::libc::SYS_linkat
            || sc == nix::libc::SYS_renameat
            || sc == nix::libc::SYS_renameat2;
        #[cfg(target_arch = "aarch64")]
        let is_at = is_at || sc == SYS_FSTATAT;
        if is_at {
...
fn ptrace_syscall_is_entry(pid: Pid, fallback: &mut bool) -> bool {
    // Linux: PTRACE_GET_SYSCALL_INFO returns whether this syscall-stop is ENTRY/EXIT/SECCOMP.
    // If unsupported, fall back to "toggle" behavior (best-effort).
    ...
    // Fallback: alternate entry/exit.
    let is_entry = !*fallback;
    *fallback = !*fallback;
    is_entry
}
